...
class TabularLegendLayout implements LegendLayout {
  ...
  Widget build(BuildContext context, List<Widget> legendEntries) {
    final paddedLegendEntries = ((cellPadding == null)
        ? legendEntries
        : legendEntries.map((entry) => Padding(padding: cellPadding!, child: entry)).toList());
  ...
  @override
  int get hashCode =>
      Object.hash(desiredMaxRows, desiredMaxColumns, isHorizontalFirst, cellPadding);
  ...
  Widget _buildHorizontalFirst(List<Widget> legendEntries) {
    ...
    for (var i = 0; i < legendEntries.length; i += maxColumns) {
      rows.add(TableRow(
          children: legendEntries.sublist(i, min(i + maxColumns, legendEntries.length)).toList()));
    }
  ...
  Table _buildTableFromRows(List<TableRow> rows) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      if (padCount > 0) {
        rowChildren.addAll(Iterable<Padding>.generate(padCount, (_) => padWidget));
      }
    }
